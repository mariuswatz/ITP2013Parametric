...
public class UFFT extends UMB {
  ...
  public void useEqualizer(boolean on) {
    ...
    if (on) {
      ...
      for (i=0; i<WS2; i++)
        _equalize[i]=scaling*(float)Math.log((double)(WS2-i)*inv_half_nfreq);
    }
  }

  // bell filter envelope to reduce artefacts caused by the edges of standard
  // filter rect
  // 0.0 < power < 2.0
    ...
    if (_isEqualized) {
      for (i=0; i<WS2; i++) {
        ...
        _fft_result[i]=_equalize[i]
            *(float)Math.sqrt(currBin[0]*currBin[0]+currBin[1]*currBin[1]);
      }
    } else {
      for (i=0; i<WS2; i++) {
        currBin=_fftBuffer[i];
